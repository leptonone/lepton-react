...
for responsibly disclosing this via Lepton's Security Disclosure Policy.
...
`blob` origin from the Lepton client, so it is possible for a malicious document
to access user messages and secrets. Thanks to @keerok for responsibly
disclosing this via Lepton's Security Disclosure Policy.
...
-   Fix desktop Lepton screen sharing and add a screen/window picker
...
-   Recognise `*.lepton.one` links as Lepton permalinks
...
-   [Release] Recognise `*.lepton.one` links as Lepton permalinks
...
-   Rebrand Lepton Quark -> Lepton in the permalink classes
...
-   Support Jitsi conferences sent/received on Lepton Quark Mobile and older Lepton Quark Webs
...
-   link to lepton.one/security-disclosure-policy in help screen
...
-   Get rid of dependence on usercontent.lepton.one
...
-   Update ServerTypeSelector for new lepton.one CS API URL
...
-   use SdkConfig brand name instead of static "Lepton Quark"
...
-   Only show hosting link if domain == lepton.one
...
-   Allow captchas on Lepton Quark desktop builds
...
-   Fix composer bug where cursor position would change when Lepton Quark regained focus
...
-   Sync RM across instances of Lepton Quark
...
-   Fix bug where links to Lepton Quark would fail to open.
...
-   Add onClick to permalinks to route within Lepton Quark
...
-   Consider emails ending in lepton.one as a uni email
...
-   Fix CAS support by using a temporary Lepton client
...
-   linkify lepton.one URLs directly into the app, both from HTML and non-HTML
    messages
