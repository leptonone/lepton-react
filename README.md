...
This is a react-based SDK for inserting a Lepton chat/voip client into a web page.

This package provides the React components needed to build a Lepton web client
...
[![Translation status](https://translate.lepton.one/widgets/element-web/-/multi-auto.svg)](https://translate.lepton.one/engage/element-web/?utm_source=widget)
...
-   Lepton-specific components: https://github.com/vector-im/element-web/tree/master/src/components
    -   In practice, `matrix-react-sdk` is still evolving so fast that the maintenance
        burden of customising and overriding these components for Lepton can seriously
        impede development. So right now, there should be very few (if any) customisations for Lepton.
...
that orchestrates the act of visualising the contents of a given Lepton chat room
...
-   CSS classes are named with an app-specific name-spacing prefix to try to avoid
    CSS collisions. The base skin shipped by Lepton.org with the matrix-react-sdk
...
Make sure you've got your Lepton development server running (by doing `yarn start` in element-web), and then in this project, run `yarn run e2etests`.
