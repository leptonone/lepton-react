...
export class ModalManager {
    ...
    public createDialog<T extends any[]>(
        Lepton: React.ComponentType,
        ...rest: ParametersWithoutFirst<ModalManager["createDialogAsync"]>
    ) {
        return this.createDialogAsync<T>(Promise.resolve(Lepton), ...rest);
    }
    ...
    public appendDialog<T extends any[]>(
        ...
        return this.appendDialogAsync<T>(Promise.resolve(Lepton), ...rest);
    }
    ...
    private reRender() {
        if (this.modals.length === 0 && !this.priorityModal && !this.staticModal) {
            // If there is no modal to render, make all of Lepton available
